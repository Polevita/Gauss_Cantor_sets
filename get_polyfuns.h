...
void littlematrix(arb_mat_t smallmatrix, const slong *multmatrix, const slong *mapmats, const slong *comulti, bool_mat_t temp, const arb_t beta, const slong m, const arb_ptr nuls, const slong nunique, const slong total, const slong maxmul, slong prec)  {

    slong j, k, k0, k1, k2, cw;
    ...
    for (k = 0; k<nunique; k++) {
        // arb_mat_zero(t0); 
        for (k0 = 1; k0 < multmatrix[k*(maxmul+1)]-1; k0++) {
            cw = multmatrix[k*(maxmul+1)+k0];
            // flint_printf("%wd --- cw \n", cw);
            arb_mat_zero(bmatrix);
            for (k2 = 0; k2<m; k2++) { 
                difdynsys(z,nuls+k2,(mapmats+(cw-1)*4),prec);
            ...
            for  (j = 0; j<nunique; j++) {
               if (bool_mat_get_entry(temp,j,comulti[cw-1]) > 0) {
                   arb_mat_window_add(smallmatrix,bmatrix,j*m,(j+1)*m,k*m,(k+1)*m,prec); 
...
int vector_positive (const arb_poly_t *testfuns, slong logpart, slong dim, const arf_t a, const arf_t b, slong prec) { 
...
for (k0=1; k0<npart+1; k0++) {
    ...
    //     arb_poly_printd(*(testfuns+k1),10);
    //    flint_printf(" --- polynomial #%wd \n", k0);
        if (arb_poly_is_zero(*(testfuns+k1))==0) {
            arb_poly_evaluate(x1,*(testfuns+k1),x0,prec);
...
int get_eig_poly(arb_poly_t *res, slong *params, const arb_ptr Vq, const arb_ptr nuls, const slong m,  const slong nunique, const arf_t a, const arf_t b) {

  slong k0, prec, logpart, print_dig=16; 
  int sw;
  ...
  for (k0=0; k0<nunique; k0++) {
      arb_poly_fit_length(*(res+k0),m);
      test_fun_poly(*(res+k0),Vq+k0*m,nuls,m,prec);
      arb_poly_printd(*(res+k0),print_dig);
      flint_printf(" --- polynomial #%wd \n", k0+1);
  }
  
  sw = vector_positive(res,logpart,nunique,a,b,prec);
...
int leading_vector(arb_poly_t *res, arb_t rlam, slong *params, const slong *multmatrix, const slong *mapmats, const slong *comulti, bool_mat_t temp, const arb_t beta, const slong m, const arb_ptr nuls, const slong nunique, const slong total, const slong maxmul, const arf_t a, const arf_t b)  {
  ...
  //  acb_mat_t BC, L, R;
    ...
    littlematrix(B, multmatrix, mapmats, comulti, temp, beta, m, nuls, nunique, total, maxmul, prec);
 ...
 //   flint_printf("--- BP(1,1)\n");
    ...
    arb_printd(arb_mat_entry(v1,k0,0),30);
    flint_printf("\t --- v1 \n");
    arb_printd(arb_mat_entry(v0,k0,0),30);
    flint_printf("\t --- v0 \n");

    arb_div(lam0,arb_mat_entry(v1,k0,0),arb_mat_entry(v0,k0,0),prec);

    flint_printf("Pong! The leading eigenvalue: \n");
    arb_printd(lam0, 20); 
    flint_printf("\n");
    ...
    k0 = MAX(-k0,6);
    flint_printf(" \n Required minimal m:  k0 = %wd \n",k0); 
    if (m < k0) {
        flint_printf(" Increase m \n"); 
        arb_clear(t0); arb_clear(nv0);  arb_clear(lam0);
    ...
    arb_mul_ui(nv0,nv0,20,prec); // experimental: -1 --> -6 
    ...
    if (arf_cmp(anorm,eps)>0) { 
        ...
        if (arf_cmp(anorm,eps)>0) {  // 12 iterations didn't help -- need more iterations
            ...
            sw = get_eig_poly(res,params,Vq,nuls,m,nunique,a,b);
