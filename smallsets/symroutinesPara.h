...
void function_Nbn(arb_mat_t *res, const arb_poly_t *pqn, const arb_ptr z0, const arb_ptr baz0, const slong totalz0, 
                  const slong ndif, const slong *mapmatrix, const arb_ptr pows, const bool_mat_t minimatrix, 
                  slong *comul, slong *mul, const slong d, const slong dim, const slong total, const slong prec)   {
// function_Nbn(numdiv, pqn, z0, baz0, npart, mapmatrix, pows, minimatrix, comul, mul, d, dim, total, prec); 
...
// pows = -2q-d, -2q-d-1, \ldots, -2q - d - (d+1)/2+1
    ...
    omp_lock_t *lock = (omp_lock_t *)calloc(dim, sizeof(omp_lock_t));
    slong k00; 
    for (k00 = 0; k00<dim; k00++) {
      arb_mat_zero(*(res+k00));
      omp_init_lock(lock+k00);
     }
    ...
    #pragma omp parallel shared(res,pqn,mapmatrix,minimatrix,comul,mul,lock)
   {
...
//       flint_printf("thread #= %wd \n", omp_get_thread_num());
       ...
       function_San0(s0,lin,*(pqn+mul[k0]),(mapmatrix+4*k0),d,prec,temp1,temp2);
       ...
       for (k2 = 0; k2<totalz0; k2++)  {
           ...
           for (k3 = 0; k3<dim; k3 ++) {    
               if (bool_mat_get_entry(minimatrix,k3,comul[k0])>0)  
//               #pragma omp atomic
               {
                   omp_set_lock(lock+kk3);
                   arb_addmul(arb_mat_entry(*(res+k3),k2,0),x1,arb_mat_entry(xv1,k2,0),prec);
                   omp_unset_lock(lock+k3);
               }
           }
       }
       ...
       for (k3 = 0; k3< dim; k3++) 
            if (bool_mat_get_entry(minimatrix,k3,comul[k0])>0)  {
                function_Panbn0(p0,lin,s0,s1,*(pqn+k3),prec,temp1,temp2);
                for (k1 = 1; k1< ndif-1; k1++)  {
                    for (k2 = 0; k2<totalz0; k2++)  {
                        arb_poly_evaluate(x1,p0,z0+k2,prec);
//                        #pragma omp atomic
                        omp_set_lock(lock+k3);
                        arb_addmul(arb_mat_entry(*(res+k3),k2,k1),x1,arb_mat_entry(xv1,k2,k1),prec);
                        omp_unset_lock(lock+k3);
                    }
                ...
                for (k2 = 0; k2<totalz0; k2++)  {
                    ...
                    omp_set_lock(lock+k3);
                    arb_addmul(arb_mat_entry(*(res+k3),k2,k1),x1,xv0+k2,prec);
                    omp_unset_lock(lock+k3);
                } //k2
            } // if
       } //k3
      // if (( k0 % 5000) == 0)  { flint_printf(" Parsed %wd functions\n", k0); }
    } //k0
...
int ratbounds(arf_ptr bb, slong logpart, arb_poly_t *pqn, const arb_t q, const arb_ptr nuls, const arf_t a, const arf_t b, slong d, slong dim, bool_mat_t minimatrix, slong *mapmatrix, slong *comul, slong *mul, slong total, slong prec) {
    ...
    function_Nbn(numdiv, pqn, z0, baz0, npart, ndif, mapmatrix, pows, minimatrix, comul, mul, d, dim, total, prec); 
    
    for (k0 = 0; k0 < dim; k0++) { 

        flint_printf("\n * * * \n Estimates for the function = %wd \n \n", k0);

        if (arb_poly_is_zero(*(pqn+k0)) == 0 ) {

            for (k1 = 0; k1 < npart; k1++) { 
                for (k2 = 1; k2 < ndif; k2++) {
                    arb_set(nb+k2-1,arb_mat_entry(*(numdiv+k0),k1,k2)); 
                }
                arb_poly_evaluate(x1,*(pqn+k0),z0+k1,prec);  //denominator

                arb_div(x1,arb_mat_entry(*(numdiv+k0),k1,0),x1,prec); //value of the function [LQ]_k0/Q_k0 at the centre
                ...
                arb_poly_evaluate(x2,*(pqn+k0),baz0+k1,prec); //denominator on a ball   
                ...
                if ( arb_contains_si(x1,1) !=0 ) {
                    ...
                    for (k0 = 0; k0 < dim; k0++) {        arb_mat_clear(*(numdiv+k0) );      }
                    return 1;
                }
            ...
            flint_printf(" function # %wd   vanished, no estimate is needed", k0);
        }   // functions if 
    } // functions for 
